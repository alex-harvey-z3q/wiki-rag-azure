#!/usr/bin/env bash
#
# Deploy the indexer ECS scheduled task from GitHub Actions.
#
# Requires:
...
get_eventbridge_target_json() {
  aws events list-targets-by-rule \
    --rule "$INDEXER_RULE_NAME" \
    --query "Targets[?Id=='$TARGET_ID'] | [0]" \
    --output json
}
